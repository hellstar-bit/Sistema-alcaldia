// frontend/src/components/gestion/IPSFormModal.tsx
...
import { ipsAPI, gestionUtils, type IPSDetailed, type CreateIPSRequest, type UpdateIPSRequest } from '../../services/gestionApi';
...
interface IPSFormModalProps {
  isOpen: boolean;
  onClose: () => void;
  editingIPS?: IPSDetailed | null;
  onSuccess: () => void;
}
...
export const IPSFormModal: React.FC<IPSFormModalProps> = ({
  isOpen,
  onClose,
  editingIPS,
  onSuccess
  ...
  const isEditing = !!editingIPS;

  useEffect(() => {
    if (isOpen) {
      if (editingIPS) {
        setFormData({
          nombre: editingIPS.nombre || '',
          codigo: editingIPS.codigo || '',
          descripcion: editingIPS.descripcion || '',
          direccion: editingIPS.direccion || '',
          telefono: editingIPS.telefono || '',
          email: editingIPS.email || '',
          contacto: editingIPS.contacto || '',
          tipoServicio: editingIPS.tipoServicio || '',
          activa: editingIPS.activa
        });
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (name === 'nombre' && generateCode && !isEditing) {
      const generatedCode = gestionUtils.generateIPSCode(value);
      setFormData(prev => ({
        ...prev,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    showLoading(
      isEditing ? 'Actualizando IPS...' : 'Creando IPS...',
      'Guardando información'
    );

    try {
      if (isEditing) {
        const updateData: UpdateIPSRequest = {
        ...
        const response = await ipsAPI.update(editingIPS!.id, updateData);
        
        close();
        
        if (response.success) {
          showSuccess('IPS actualizada', {
            title: '¡IPS actualizada!',
            text: 'Los datos de la IPS han sido actualizados exitosamente'
          });
          onSuccess();
        } else {
          showError({
        ...
        const createData: CreateIPSRequest = {
          nombre: formData.nombre.trim(),
          codigo: formData.codigo.trim(),
          descripcion: formData.descripcion.trim() || undefined,
          direccion: formData.direccion.trim() || undefined,
        ...
        const response = await ipsAPI.create(createData);
        
        close();
        
        if (response.success) {
          showSuccess('IPS creada', {
            title: '¡IPS creada exitosamente!',
            text: 'La nueva IPS ha sido registrada en el sistema'
          });
          onSuccess();
        } else {
          showError({
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-xl font-bold">
                  {isEditing ? 'Editar IPS' : 'Nueva IPS'}
                </h2>
                <p className="text-blue-100 text-sm">
                  {isEditing ? 'Modificar información de la IPS' : 'Registrar nueva Institución Prestadora de Servicios de Salud'}
                </p>
              </div>
            </div>
            <button 
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="md:col-span-2">
                  ...
                  <input
                    ...
                    placeholder="Ej: Hospital San José, Clínica Los Andes..."
                    disabled={loading}
                  />
                  {errors.nombre && (
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <TagIcon className="w-4 h-4 text-gray-400 absolute right-10 top-1/2 transform -translate-y-1/2 pointer-events-none" />
                  </div>
                </div>
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea
                    ...
                    placeholder="Descripción opcional de la IPS..."
                    disabled={loading}
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="md:col-span-2">
                  ...
                  <input
                    ...
                    onChange={handleInputChange}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors"
                    placeholder="Dirección de la IPS"
                    disabled={loading}
                  />
                </div>
          ...
          <div className="mt-8 flex flex-col sm:flex-row items-center justify-end space-y-3 sm:space-y-0 sm:space-x-3 pt-6 border-t border-gray-200">
            <button
              ...
              onClick={handleClose}
              disabled={loading}
              className="w-full sm:w-auto bg-primary-600 hover:bg-primary-700 text-white px-6 py-3 rounded-lg transition-colors font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2"
            >
              Cancelar
            </button>
            <button
            ...
            >
              {loading ? (
                ...
                <>
                  ...
                  <span>{isEditing ? 'Actualizar IPS' : 'Crear IPS'}</span>
                </>
              )}
            </button>
          </div>
