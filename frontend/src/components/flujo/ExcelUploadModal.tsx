// frontend/src/components/flujo/ExcelUploadModal.tsx
...
import { flujoAPI, flujoUtils } from '../../services/flujoApi';
import { useSweetAlert } from '../../hooks/useSweetAlert';

interface FlujoExcelUploadModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const FlujoExcelUploadModal: React.FC<FlujoExcelUploadModalProps> = ({
  isOpen,
  onClose,
  selectedEPS,
  selectedPeriodo,
  ...
  const handleFileSelect = async (file: File) => {
    const validation = flujoUtils.validateExcelFile(file);
    ...
    const structureValidation = await flujoUtils.validateExcelStructure(file);
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await flujoAPI.uploadExcel(
        selectedFile,
        selectedEPS.id,
        selectedPeriodo.id
      );
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      const blob = await flujoAPI.downloadPlantilla();
      flujoUtils.downloadBlob(blob, 'Plantilla_Flujo.xlsx');
        ...
        <div className="bg-gradient-to-r from-blue-900 to-blue-800 p-6 text-white">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-xl font-bold">Importar Datos de Flujo</h2>
                <p className="text-blue-100 text-sm">Cargar información desde archivo Excel</p>
              </div>
            </div>
            <button 
              onClick={onClose}
            ...
            <div className="bg-gradient-to-br from-blue-50 to-blue-100 border border-blue-200 rounded-xl p-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-lg font-bold text-blue-900">{selectedEPS?.nombre || 'No seleccionada'}</p>
                </div>
              </div>
            </div>
              ...
              <div>
                ...
                <ul className="text-sm text-info-800 space-y-1">
                  ...
                  <li className="ml-4 font-mono text-xs bg-white px-2 py-1 rounded">
                    Prestador | Incremento | Tipo Contrato | Fecha contrato | Valor facturado | Valor Glosa | Vr reconocido | Valor Pagado | Fecha pago | Saldo adeudado | Saldo Total | Origen del Giro
                  </li>
                  ...
                  <li>• Los valores monetarios deben ser números sin formato (sin comas ni símbolos)</li>
